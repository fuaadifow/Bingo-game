...
public class BingoCard {
  ...
  public String getCardNumbers() {
    ...
    int x;
    int y;
    for (x = 0; x<getNumberOfRows();x++) {
      for (y = 0; y <getNumberOfColumns(); y++)
        sb.append(numbers[x][y]).append(Defaults.getNumberSeparator());
    }
  ...
  public void setCardNumbers(String[] numbersAsStrings) {
    ...
    for (x = 0; x < getNumberOfRows(); x++)
      if (getNumberOfColumns() >= 0)
        System.arraycopy(numbersList, (x * getNumberOfColumns()), this.numbers[x], 0, getNumberOfColumns());
    /* TODO
  ...
  public boolean isWinner() {
    ...
    return markedW == (getNumberOfColumns() * getNumberOfRows());
  }
}